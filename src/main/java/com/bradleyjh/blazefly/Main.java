...
public class Main extends JavaPlugin implements Listener {
	...
	public String fuelBlock(Player player) {
		if (hasPermission(player, "vip")) {
			return getConfig().getString("VIPBlock").toUpperCase();
		} else {
			return getConfig().getString("fuelBlock").toUpperCase();
		}
	}
	...
	public Integer fuelSubdata(Player player) {
		if (hasPermission(player, "vip")) {
			return getConfig().getInt("VIPSubdata");
		} else {
			return getConfig().getInt("fuelSubdata");
		}
	}

	// Get the time each fuel block should last (-1 because Timer adds a second at
	// 0)
	public Double fuelTime(Player player) {
		if (hasPermission(player, "vip")) {
			return getConfig().getDouble("VIPTime") - 1;
		} else {
			return getConfig().getDouble("fuelTime") - 1;
		}
	}
		...
		if (commandLabel.equalsIgnoreCase("fly") || commandLabel.equalsIgnoreCase("bfly")) {
			...
			// Enable flight
				...
				if (core.hasFuel(player, fuelBlock(player), fuelSubdata(player)) || core.hasFuelCount(player)) {
					...
					if (hasPermission(player, "vip")) {
						fuelName = getConfig().getString("VIPName");
					}
					HashMap<String, String> keywords = new HashMap<>();
